...
export default function TargetCard(props: TargetCardProps) {
	...
	const subtitle = createMemo(() => {
		...
		if (recording) {
			return recording.mode === "studio"
				? t("newMain.targetCard.studioMode")
				: t("newMain.targetCard.instantMode");
		}
		return undefined;
	});
	...
	const handleCopy = async (e: MouseEvent) => {
		...
		try {
			...
			toast.success(t("newMain.targetCard.copiedToast"));
		} catch (error) {
			console.error("Failed to copy screenshot:", error);
			toast.error(t("newMain.targetCard.copyFailedToast"));
		}
	};
	...
	const handleSave = async (e: MouseEvent) => {
		...
		try {
			const path = await save({
				...
				filters: [
					{
						name: t("newMain.targetCard.imageFilterName"),
						extensions: ["png"],
					},
				],
			});
			...
			toast.success(t("newMain.targetCard.savedToast"));
		} catch (error) {
			console.error("Failed to save screenshot:", error);
			toast.error(t("newMain.targetCard.saveFailedToast"));
		}
	};
	...
	const handleOpenRecordingFolder = (e: MouseEvent) => {
		...
		openRecordingFolder(recording.path, recording.mode).catch((error) => {
			...
			toast.error(t("newMain.targetCard.openFolderFailedToast"));
		});
	};
	...
	const handleDeleteRecording = async (e: MouseEvent) => {
		...
		const recording = recordingTarget();
		if (!recording) return;
		if (!(await ask(t("newMain.targetCard.deleteConfirm")))) return;
			...
			<div class="relative h-19 w-full overflow-hidden bg-gray-4/40">
				...
				>
					{(src) => (
						<img
							...
							alt={
								local.variant === "display"
									? t("newMain.targetCard.displayPreview", {
											name: label() ?? "",
										})
									: t("newMain.targetCard.windowPreview", {
											name: label() ?? "",
										})
							}
				...
				<Show when={appIconSrc()}>
					{(src) => (
						<div class="flex absolute inset-0 justify-center items-center pointer-events-none bg-black/45">
							<img
								src={src()}
								alt={t("newMain.targetCard.iconAlt", { name: label() ?? "" })}
				...
				<Show when={(recordingTarget()?.clip_count ?? 0) > 1}>
					<div class="absolute left-1 top-1 rounded-full bg-black/55 px-1.5 py-0.5 text-[10px] font-medium text-white">
						{t("newMain.targetCard.clips", {
							n: recordingTarget()?.clip_count ?? 0,
						})}
					</div>
				</Show>
					...
					<div class="absolute inset-0 flex items-center justify-center bg-black/75">
						<div class="flex items-center gap-1 px-1.5 py-0.5 rounded-sm bg-red-9/20 text-red-11">
							...
							<span class="text-[10px] font-medium">
								{recordingFailed()
									? t("newMain.targetCard.recordingFailed")
									: t("newMain.targetCard.uploadFailed")}
							</span>
						</div>
					</div>
				</Show>
			...
			<div class="flex flex-col w-full">
				...
				<Show when={local.variant === "screenshot"}>
					<div class="flex items-center justify-between px-2 pb-1.5 pt-0.5 gap-1">
						<Tooltip content={t("newMain.targetCard.edit")}>
							<div
								role="button"
								tabIndex={-1}
						...
						<Tooltip content={t("newMain.targetCard.copyToClipboard")}>
							<div
								role="button"
								tabIndex={-1}
								onClick={handleCopy}
						...
						<Tooltip content={t("newMain.targetCard.saveAs")}>
							<div
								role="button"
								tabIndex={-1}
								onClick={handleSave}
						...
						return (
							<div class="flex items-center justify-between px-2 pb-1.5 pt-0.5 gap-1">
									...
									<Show
										...
										fallback={
											<Tooltip
												content={
													uploadFailed
														? t("newMain.targetCard.retryUpload")
														: t("newMain.targetCard.reupload")
												}
											>
												<div
													role="button"
								...
								<Show when={recording.sharing}>
									<Tooltip content={t("newMain.targetCard.openLink")}>
										<div
											role="button"
											tabIndex={-1}
								...
								<Tooltip content={t("newMain.targetCard.openFolder")}>
									<div
										role="button"
										tabIndex={-1}
								...
								<Tooltip content={t("newMain.targetCard.delete")}>
									<div
										role="button"
										tabIndex={-1}
