...
export default function CameraSelect(props: {
	...
	const label = () =>
		...
		(hasSelection() ? t("newMain.devices.camera") : t("newMain.devices.noCamera"));
				...
				<div class={DEVICE_ROW_TRAILING_CLASS}>
					<Show when={showHiddenIndicator()}>
						<button
							...
							title={t("newMain.devices.showCameraPreview")}
							aria-label={t("newMain.devices.showCameraPreview")}
						>
					...
					<Show when={showSettingsShortcut()}>
						<button
							...
							onPointerDown={(e) => e.stopPropagation()}
							class={DEVICE_SHORTCUT_BUTTON_CLASS}
							title={t("newMain.devices.cameraSettings")}
							aria-label={t("newMain.devices.cameraSettings")}
						>
		...
		<div class="flex flex-col gap-1 items-stretch text-(--text-primary)">
			<button
				...
				onClick={() => {
					...
					Promise.all([
						CheckMenuItem.new({
							text: t("newMain.devices.noCamera"),
			...
			>
				...
				<p class="flex-1 text-sm text-left truncate">
					{props.value?.display_name ?? t("newMain.devices.noCamera")}
				</p>
