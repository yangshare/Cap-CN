...
function BrandSettingsDialog(props: {
	...
	const selectLogoFile = (file: File) => {
		if (!isSupportedLogoContentType(file.type)) {
			toast.error(t("editor.org.logoUnsupportedType"));
			return;
		}
	...
	const save = async () => {
		...
		try {
			...
			toast.success(t("editor.org.brandingUpdated"));
			...
			toast.error(
				error instanceof Error
					? error.message
					: t("editor.org.brandingUpdateFailed"),
			);
		} finally {
			setSaving(false);
		}
	...
	return (
		<Dialog.Root open={props.open} onOpenChange={props.onOpenChange} size="sm">
			<DialogContent
				title={props.organization?.name ?? t("editor.org.organizationDefault")}
				...
				confirm={
					<>
						...
						<Dialog.ConfirmButton
							...
							onClick={() => void save()}
						>
							{saving() ? t("editor.org.saving") : t("editor.org.save")}
						</Dialog.ConfirmButton>
					</>
				}
			>
...
export function OrganizationDropdown() {
	...
	const triggerLabel = createMemo(() => {
		...
		if (availability === "available") {
			return selectedOrganization()?.name ?? t("editor.org.organizationDefault");
		}
		if (availability === "loading") return t("editor.org.loading");
		if (availability === "unavailable") return t("editor.org.organizationDefault");
		return t("editor.org.signIn");
	});
	const fallbackTitle = createMemo(() => {
		const availability = organizationSelection.availability();
		if (availability === "loading") return t("editor.org.loadingOrganizationsTitle");
		if (availability === "unavailable") return t("editor.org.loadOrganizationsFailedTitle");
		return t("editor.org.brandingRequiresSignInTitle");
	});
	const fallbackDescription = createMemo(() => {
		const availability = organizationSelection.availability();
		if (availability === "loading") {
			return t("editor.org.fetchingBrandingDescription");
		}
		if (availability === "unavailable") {
			return t("editor.org.unavailableDescription");
		}
		return t("editor.org.signedOutDescription");
	});
