...
export function LayersPanel() {
	...
	const annotationTypeLabel = (type: Annotation["type"]) =>
		t(`screenshotEditor.tools.${type}`);
	...
	const getTypeLabel = (ann: Annotation) => {
		...
		return annotationTypeLabel(ann.type);
	};
	...
	return (
		<div class="flex flex-col h-full w-56 border-r border-gray-3 bg-gray-1 dark:bg-gray-2 select-none z-20">
					...
					fallback={
						<div class="flex flex-col items-center justify-center h-full px-4 text-center">
							...
							<p class="text-xs text-gray-10">
								{t("screenshotEditor.layers.noLayersYet")}
							</p>
							<p class="text-[10px] text-gray-8 mt-1">
								{t("screenshotEditor.layers.useToolsHint")}
							</p>
						</div>
					}
				>
			...
			<div class="px-3 py-2 border-t border-gray-3 text-[10px] text-gray-9">
				{t("screenshotEditor.layers.reorderHint")}
			</div>
		</div>
	);
}
